...
const SEED_DATA = {
  ...
  quick_actions: [
    { id: "qa-1", label: "Add Package", path: "/admin/dashboard", route: "Packages", iconName: "Package", bg: "bg-blue-50", text: "text-blue-600", _sort_order: 0 },
    { id: "qa-2", label: "Add Promo", path: "/admin/dashboard", route: "Offers", iconName: "Tag", bg: "bg-violet-50", text: "text-violet-600", _sort_order: 1 }
  ],
  packages_list: [
...
(async () => {
  ...
  for (const table of Object.keys(TABLES_SCHEMAS)) {
  ...
  for (const [table, items] of Object.entries(SEED_DATA)) {
    if (items.length > 0) {
      ...
      console.log(`Seeded table: ${table}`);
    }
  }
