...
export const Avatar = ({
    ...
    const renderMainContent = () => {
        ...
        if (initials) {
            return <span className={cx("text-slate-600 dark:text-slate-350 font-bold", styles[size].initials)}>{initials}</span>;
        }

        if (PlaceholderIcon) {
            return <PlaceholderIcon className={cx("text-slate-400 dark:text-slate-500", styles[size].icon)} />;
        }

        return placeholder || <User01 className={cx("text-slate-400 dark:text-slate-500", styles[size].icon)} />;
    };
    ...
    return (
        ...
        >
            <div
                className={cx(
                    "relative inline-flex size-full shrink-0 items-center justify-center overflow-hidden rounded-md bg-slate-100 dark:bg-slate-800 outline-[0.5px] -outline-offset-[0.5px] outline-black/16 before:inset-[0.5px]",
                    rounded && "rounded-full",
                    canShowImage &&
                        size !== "xs" &&
