...
const TableCardRoot = ({ children, className, size = "md", ...props }: HTMLAttributes<HTMLDivElement> & { size?: "sm" | "md" }) => {
    return (
        <TableContext.Provider value={{ size }}>
            <div {...props} className={cx("overflow-hidden rounded-3xl bg-white dark:bg-[#0e162f] shadow-xs border border-slate-100 dark:border-slate-800/80", className)}>
                {children}
            </div>
...
const TableCardHeader = ({ title, badge, description, contentTrailing, className }: TableCardHeaderProps) => {
    ...
    return (
        <div
            className={cx(
                "relative flex flex-col items-start gap-4 border-b border-slate-100 dark:border-slate-850 bg-white dark:bg-[#0e162f] px-4 md:flex-row",
        ...
        >
            <div className="flex flex-1 flex-col gap-0.5">
                <div className="flex items-center gap-2">
                    <h2 className="text-md font-bold text-slate-900 dark:text-white">{title}</h2>
                    {badge ? (
...
const TableHeader = <T extends object>({ columns, children, bordered = true, className, size: sizeProp, ...props }: TableHeaderProps<T>) => {
    ...
    return (
        <AriaTableHeader
            ...
            className={(state) =>
                cx(
                    "relative bg-slate-50/75 dark:bg-slate-900/30",
                    size === "sm" ? "h-9" : "h-11",

                    // Row border—using an "after" pseudo-element to avoid the border taking up space.
                    bordered &&
                        "[&>tr>th]:after:pointer-events-none [&>tr>th]:after:absolute [&>tr>th]:after:inset-x-0 [&>tr>th]:after:bottom-0 [&>tr>th]:after:h-px [&>tr>th]:after:bg-slate-100 dark:[&>tr>th]:after:bg-slate-800 [&>tr>th]:focus-visible:after:bg-transparent",
...
const TableHead = ({ className, tooltip, label, children, ...props }: TableHeadProps) => {
    ...
    const isRightAligned = typeof className === "string" && (className.includes("text-right") || className.includes("justify-end"));
    ...
    return (
        ...
        >
            {(state) => (
                <AriaGroup className={cx("flex items-center gap-1", isRightAligned && "justify-end w-full")}>
                    <div className={cx("flex items-center gap-1", isRightAligned && "justify-end")}>
                        {label && <span className="text-xs font-semibold whitespace-nowrap text-slate-400 dark:text-slate-500">{label}</span>}
                        {typeof children === "function" ? children(state) : children}
                    </div>

                    {tooltip && (
                        <Tooltip title={tooltip} placement="top">
                            <TooltipTrigger className="cursor-pointer text-slate-400 dark:text-slate-500 transition duration-100 ease-linear hover:text-slate-500 focus:text-slate-500">
                    ...
                    {state.allowsSorting &&
                        (state.sortDirection ? (
                            <ArrowDown className={cx("size-3 stroke-[3px] text-slate-400 dark:text-slate-500", state.sortDirection === "ascending" && "rotate-180")} />
                        ) : (
                            <ChevronSelectorVertical size={12} strokeWidth={3} className="text-slate-400 dark:text-slate-500" />
                        ))}
                </AriaGroup>
            )}
        </AriaColumn>
...
const TableRow = <T extends object>({ columns, children, className, highlightSelectedRow = true, size: sizeProp, ...props }: TableRowProps<T>) => {
    ...
    return (
        <AriaRow
            ...
            className={(state) =>
                cx(
                    "relative outline-focus-ring transition-colors after:pointer-events-none hover:bg-slate-50/50 dark:hover:bg-slate-800/40 bg-white dark:bg-[#0e162f]",
                    size === "sm" ? "h-14" : "h-18",
                    highlightSelectedRow && "selected:bg-slate-50/50 dark:selected:bg-slate-800/40",

                    // Row border—using an "after" pseudo-element to avoid the border taking up space.
                    "[&>td]:after:absolute [&>td]:after:inset-x-0 [&>td]:after:bottom-0 [&>td]:after:h-px [&>td]:after:w-full [&>td]:after:bg-slate-100 dark:[&>td]:after:bg-slate-800 last:[&>td]:after:hidden [&>td]:focus-visible:after:opacity-0 focus-visible:[&>td]:after:opacity-0",
...
const TableCell = ({ className, children, size: sizeProp, ...props }: TableCellProps) => {
    ...
    return (
        <AriaCell
            ...
            className={(state) =>
                cx(
                    "relative text-sm text-slate-700 dark:text-slate-350 outline-focus-ring focus-visible:z-1 focus-visible:outline-2 focus-visible:-outline-offset-2",
                    size === "sm" && "px-5 py-3",
                    size === "md" && "px-6 py-4",
