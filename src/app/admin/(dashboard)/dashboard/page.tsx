          ...
          {activeTab === "Overview" && (
            <div className="space-y-6">
              ...
              <div className="space-y-4 pt-2">
                <h2 className="text-xl font-bold text-slate-800 tracking-tight">System status</h2>
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">

                  {/* Hero overlay */}
                  <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm flex items-start gap-4 overflow-hidden relative">
                    <div className="absolute left-0 top-0 bottom-0 w-1 rounded-l-2xl bg-gradient-to-b from-emerald-400 to-teal-500" />
                    <div className={`p-2.5 rounded-xl ${homeSections.hero ? "bg-emerald-50 text-emerald-600" : "bg-slate-100 text-slate-400"} shrink-0`}>
                      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                      </svg>
                    </div>
                    <div className="flex flex-col min-w-0">
                      <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Hero overlay</span>
                      <span className={`text-2xl font-extrabold mt-0.5 ${homeSections.hero ? "text-emerald-600" : "text-slate-500"}`}>
                        {homeSections.hero ? "ON" : "OFF"}
                      </span>
                      <span className={`text-[11px] font-semibold mt-0.5 ${homeSections.hero ? "text-emerald-400" : "text-slate-300"}`}>
                        {homeSections.hero ? "● Active" : "○ Disabled"}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm flex items-start gap-4 overflow-hidden relative">
                    ...
                    <div className="flex flex-col min-w-0">
                      <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Active packages</span>
                      <span className="text-2xl font-extrabold text-slate-900 mt-0.5">{countPackages}</span>
                      <span className="text-[11px] text-slate-400 font-medium mt-0.5">Published plans</span>
                    </div>
                  </div>
                  ...
                  <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm flex items-start gap-4 overflow-hidden relative">
                    ...
                    <div className="flex flex-col min-w-0">
                      <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Active FAQs</span>
                      <span className="text-2xl font-extrabold text-slate-900 mt-0.5">{faqs.filter((f) => f.isPublished).length || 0}</span>
                      <span className="text-[11px] text-slate-400 font-medium mt-0.5">Visible on site</span>
                    </div>
                  </div>

                  {/* Total admins */}
                  <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm flex items-start gap-4 overflow-hidden relative">
                    <div className="absolute left-0 top-0 bottom-0 w-1 rounded-l-2xl bg-gradient-to-b from-violet-400 to-purple-600" />
                    <div className="p-2.5 rounded-xl bg-violet-50 text-violet-600 shrink-0">
                      <UserCog className="w-5 h-5" />
                    </div>
                    <div className="flex flex-col min-w-0">
                      <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Total admins</span>
                      <span className="text-2xl font-extrabold text-slate-900 mt-0.5">{adminUsers.length || 1}</span>
                      <span className="text-[11px] text-slate-400 font-medium mt-0.5">Active accounts</span>
                    </div>
                  </div>
