    ...
    <div className="min-h-screen -mt-24 bg-white text-slate-800 flex overflow-hidden notranslate relative">
      ...
      <main className="flex-1 min-w-0 h-screen flex flex-col bg-slate-50/40 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto flex flex-col admin-main-scroll">
          <div className="bg-[#f1f5f9]/70 border-b border-slate-200/50 px-3 sm:px-6 lg:px-8 py-2 sm:py-3 flex items-center min-w-0">
                ...
                <div className="flex flex-wrap gap-2 items-center">
                  {Array.from({ length: 8 }).map((_, index) => (
                    <div
                      ...
                      className="h-7 w-28 rounded-full border border-slate-200 bg-white shadow-xs animate-pulse shrink-0"
                    />
                  ))}
                </div>
              ) : (
                <>
                  {/* Desktop View: Standard Flex Wrap */}
                    ...
                    {quickActions.map((item) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center gap-1.5 px-3 py-1.5 border rounded-full text-[11px] font-semibold transition-all cursor-pointer shadow-xs whitespace-nowrap ${
                            isActive
                  ...
                  {/* Mobile View: 2 Horizontal Scrollable Rows */}
                  <div className="flex md:hidden flex-col gap-1.5 overflow-x-auto pb-1 [&::-webkit-scrollbar]:hidden">
                    {/* Row 1 */}
                    <div className="flex gap-1.5 shrink-0">
                      {quickActions.filter((_, idx) => idx % 2 === 0).map((item) => {
                        const isActive = pathname === item.route || (item.label === "Packages" && pathname === "/admin/packages");
                        const ActionIcon = IconMap[item.iconName] || IconMap["Link"];
                        return (
                          <button
                            key={item.id}
                            onClick={() => router.push(item.route)}
                            className={`flex items-center gap-1 px-2.5 py-1 border rounded-full text-[10px] font-semibold transition-all cursor-pointer shadow-xs shrink-0 whitespace-nowrap ${
                              isActive
                                ? "border-brand-blue bg-blue-50/20 text-brand-blue"
                                : "border-slate-200 bg-white text-slate-700 hover:border-slate-350 hover:bg-slate-50/50"
                            }`}
                          >
                            <ActionIcon className={`w-3 h-3 ${isActive ? "text-brand-blue" : item.text}`} />
                            <span>{item.label}</span>
                          </button>
                        );
                      })}
