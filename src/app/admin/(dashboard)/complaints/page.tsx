            ...
            {complaints.length === 0 ? (
              ...
              complaints.map((c) => (
                <tr key={c.id} className="hover:bg-slate-50/70 transition-colors">
                  <td className="py-3.5"><span className="font-extrabold text-slate-800 block">{c.name}</span><span className="text-[10px] text-slate-500 font-mono">{c.clientId}</span></td>
                  <td className="py-3.5 font-mono">{c.phone}</td>
                  <td className="py-3.5 font-semibold text-brand-blue">{c.category}</td>
                  <td className="py-3.5 max-w-xs truncate text-slate-600">{c.desc}</td>
                  <td className="py-3.5 text-slate-500">{c.date}</td>
                  <td className="py-3.5"><span className={`px-2 py-0.5 rounded-full text-[9px] font-bold ${c.status === "Resolved" ? "bg-emerald-500/10 text-emerald-600 border border-emerald-500/20" : c.status === "Investigating" ? "bg-blue-500/10 text-blue-600 border border-blue-500/20" : "bg-amber-400/10 text-amber-600 border border-amber-400/20"}`}>{c.status}</span></td>
                  <td className="py-3.5 text-right space-x-2">
                    {c.status === "Pending" && <button onClick={() => updateStatus(c.id, "Investigating")} className="px-2.5 py-1 bg-blue-500/20 hover:bg-blue-500/30 text-blue-600 rounded-lg font-bold text-[10px] cursor-pointer">Investigate</button>}
                    {c.status !== "Resolved" && <button onClick={() => updateStatus(c.id, "Resolved")} className="px-2.5 py-1 bg-emerald-500/15 hover:bg-emerald-500/25 border border-emerald-500/30 text-emerald-600 rounded-lg font-bold text-[10px] cursor-pointer">Resolve</button>}
                    <button onClick={() => deleteComplaint(c.id)} className="px-2.5 py-1 border border-slate-200 hover:bg-red-500/10 hover:text-red-600 rounded-lg font-bold text-[10px] cursor-pointer">Delete</button>
                  </td>
                </tr>
              ))
            )}
