...
export default function AboutHeaderPage() {
  const router = useRouter();
  const [auth, setAuth] = useState(false);
  const [saved, setSaved] = useState(false);

  const [titleEn, setTitleEn] = useState("About ");
  const [highlightEn, setHighlightEn] = useState("M Amin Network");
  const [subtitleEn, setSubtitleEn] = useState("Discover our history, network infrastructure capabilities, and why we are South Keraniganj's most trusted broadband provider.");
  ...
  useEffect(() => {
    ...
    getSetting("page_headers").then((s) => {
      ...
      if (data && data.about_title_en) {
        setTitleEn(data.about_title_en);
        setHighlightEn(data.about_title_highlight_en || "");
        setSubtitleEn(data.about_subtitle_en || "");
      }
    });
    ...
    getSetting("about_content_full").then((s) => {
      if (s) {
        ...
        if (item && typeof item === "object") {
          ...
          if (data.headerTitleEn) setTitleEn(data.headerTitleEn);
          if (data.highlightEn) setHighlightEn(data.highlightEn);
          if (data.headerDescEn) setSubtitleEn(data.headerDescEn);
        }
      }
    });
  }, [router]);
  ...
  const save = async () => {
    ...
    const updatedHeaders = {
      ...
      about_title_bn: titleEn,
      about_title_highlight_en: highlightEn,
      about_title_highlight_bn: highlightEn,
      about_subtitle_en: subtitleEn,
      about_subtitle_bn: subtitleEn,
    };
    ...
    const updatedAbout = {
      ...currentAbout,
      headerTitleEn: titleEn,
      headerTitleBn: titleEn,
      highlightEn,
      highlightBn: highlightEn,
      headerDescEn: subtitleEn,
      headerDescBn: subtitleEn,
    };
    await setSetting("about_content_full", updatedAbout);

    setSaved(true);
    toast.success("About Page Header updated successfully!");
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="bg-white border border-slate-200/80 shadow-sm rounded-2xl p-6 space-y-6">
        <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
          ...
          <h2 className="text-base font-bold text-slate-800">About Page Header Config</h2>
        </div>
