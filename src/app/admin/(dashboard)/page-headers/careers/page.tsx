...
export default function CareersHeaderPage() {
  ...
  useEffect(() => {
    ...
    getSetting("page_headers").then((s: any) => {
      if (s) {
        setBg(s.careers_bg || "");
        setTitleEn(s.careers_title_en || "");
        setTitleBn(s.careers_title_bn || "");
        setHighlightEn(s.careers_title_highlight_en || "");
        setHighlightBn(s.careers_title_highlight_bn || "");
        setSubtitleEn(s.careers_subtitle_en || "");
        setSubtitleBn(s.careers_subtitle_bn || "");
      }
    });
  }, [router]);
  ...
  const save = async () => {
    ...
    const updated = {
      ...s,
      careers_bg: bg,
      careers_title_en: titleEn,
      careers_title_bn: titleBn,
      careers_title_highlight_en: highlightEn,
      careers_title_highlight_bn: highlightBn,
      careers_subtitle_en: subtitleEn,
      careers_subtitle_bn: subtitleBn,
    };
    await setSetting("page_headers", updated);
    setSaved(true);
    toast.success("Careers Page Header updated successfully!");
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-start justify-between bg-white border border-slate-200 shadow-sm rounded-xl p-5">
        <div>
          <h1 className="text-xl font-extrabold text-slate-900 tracking-tight">Careers Header settings</h1>
          <p className="text-xs text-slate-500 mt-0.5">
            Modify the background image/video and title/subtitle for the public careers recruitment page.
          </p>
        </div>
      ...
      <div className="bg-white border border-slate-200/80 shadow-sm rounded-2xl p-6 space-y-6">
        <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
          ...
          <h2 className="text-base font-bold text-slate-800">Careers Page Header config</h2>
        </div>
