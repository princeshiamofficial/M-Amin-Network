...
export default function OffersHeaderPage() {
  ...
  useEffect(() => {
    ...
    getSetting("page_headers").then((s: any) => {
      if (s) {
        ...
        setTitleBn(s.offers_title_bn || "");
        setHighlightEn(s.offers_title_highlight_en || "");
        setHighlightBn(s.offers_title_highlight_bn || "");
        setSubtitleEn(s.offers_subtitle_en || "");
        setSubtitleBn(s.offers_subtitle_bn || "");
      }
    });
  }, [router]);
  ...
  const save = async () => {
    ...
    const updated = {
      ...s,
      offers_bg: bg,
      offers_title_en: titleEn,
      offers_title_bn: titleBn,
      offers_title_highlight_en: highlightEn,
      offers_title_highlight_bn: highlightBn,
      offers_subtitle_en: subtitleEn,
      offers_subtitle_bn: subtitleBn,
    };
    await setSetting("page_headers", updated);
    setSaved(true);
    toast.success("Offers Page Header updated successfully!");
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-start justify-between bg-white border border-slate-200 shadow-sm rounded-xl p-5">
        <div>
          <h1 className="text-xl font-extrabold text-slate-900 tracking-tight">Offers Header settings</h1>
          <p className="text-xs text-slate-500 mt-0.5">
            Modify the background image/video and title/subtitle for the public offers promotions page.
          </p>
        </div>
      ...
      <div className="bg-white border border-slate-200/80 shadow-sm rounded-2xl p-6 space-y-6">
        <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
          ...
          <h2 className="text-base font-bold text-slate-800">Offers Page Header config</h2>
        </div>
